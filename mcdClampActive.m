function clamp = mcdClampActive(nodisp, DISP, esdiff, timestep, forcevector, kbT,v0)
    ...
    % --- 3. Relative Brownian + Active Displacements ---
    n_lib = size(DISP,1);
    idx1 = randi(n_lib, nodisp, 1);
    idx2 = randi(n_lib, nodisp, 1);
    brownian_rel = DISP(idx1,:) - DISP(idx2,:);
    
    % Calculate the magnitude of the active step for one particle
    % (Assuming v0 is your active velocity and timestep is dt)
    active_step_mag = v0 * timestep; 
    
    % Generate random 3D orientations for Particle 1 and Particle 2
    % (Since this is a Monte Carlo prediction, we assume random orientations)
    act_az1 = rand(nodisp,1) * 2 * pi;
    act_el1 = acos(2 * rand(nodisp,1) - 1) - pi/2;
    [ax1, ay1, az1] = sph2cart(act_az1, act_el1, active_step_mag);
    
    act_az2 = rand(nodisp,1) * 2 * pi;
    act_el2 = acos(2 * rand(nodisp,1) - 1) - pi/2;
    [ax2, ay2, az2] = sph2cart(act_az2, act_el2, active_step_mag);
    
    % Relative active displacement
    active_rel = [ax1 - ax2, ay1 - ay2, az1 - az2];
    
    % The total expected relative displacement during the timestep
    mcd.DISPrel = brownian_rel + active_rel;
