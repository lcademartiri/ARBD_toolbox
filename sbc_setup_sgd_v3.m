function [p,pgp,sgd_correction,sgd_edges]=sbc_setup_sgd_v3(S,PDF,nsteps)
    ...
    best_p_snapshot = p; 
    best_pgp_snapshot = []; % Populated later
    ...
    if graphing
        ...
        history.pdf_smooth = [];
        history.max_corr = [];
        history.gain = [];      % Track Gain
        history.batch_size = []; % Track Batch
        
        f_fig = figure('Units','normalized','Position',[0.1 0.1 0.6 0.8]);
        ax_dens = subplot(2,2,1);
        ax_pdf  = subplot(2,2,2);
        ax_conv = subplot(2,2,3); % Bottom Left
        ax_ctrl = subplot(2,2,4); % Bottom Right (New)
    end
    ...
    while thermflag==0 || pdf_metric_val>0.002 % Ultimate Target < 0.2%
        qs=qs+1;
        ...
        if thermflag==1 && graphing
            ...
            if mod(qs, 1000) == 0
                ...
                % --- CONTROLLER: SEARCH -> REFINE TRANSITION ---
                if pdf_metric < best_pdf_metric
                    ...
                    if current_phase == PHASE_SEARCH && pdf_metric < (convergence_target * 2.0) && qs>graceperiod
                         ...
                         % 2. Boost Batch Size (Variance Reduction)
                         sgd_batch_size = 10 * base_batch_size; 
                         
                         % 3. Cut Gain (Fine Tuning)
                         sgd_base_gain = sgd_base_gain /sqrt(10);
                         sgd_gain = sgd_base_gain;
                         
                         % 4. Reset Accumulators
                         batch_sum_drift(:) = 0; batch_counts(:) = 0; steps_in_batch = 0;
