function [p,pgp,sgd_correction,sgd_edges,history] = sbc_setup_sgd_v7(S,PDF,nsteps,opts)
% SBC_SETUP_SGD_V5
% Final v5 with:
    ...
    if thermflag == 1
        ...
        if steps_in_batch >= sgd_batch_size
            ...
            for ii = idxs'
                ...
                % --- FIX: ROBUST SNR CALCULATION ---
                if ni > 1
                    v = max(0, (ss - ni * mu^2) / (ni - 1));
                    se = sqrt(v) / sqrt(ni);
                    
                    % Only calculate SNR if we have valid variance
                    if se > 1e-15 
                        s = abs(mu) / se;
                    else
                        % Variance is effectively zero. 
                        % Either identical samples or math underflow.
                        % Treat SNR as zero to prevent plotting artifacts.
                        s = 0; 
                    end
                else
                    % Single sample: Variance is undefined. 
                    % We cannot determine Signal-to-Noise.
                    v = 0;
                    se = 0;
                    s = 0; 
                end
            ...
            % ---------------------------------------------------------
            % GAIN GOVERNOR (Global Scope)
            % ---------------------------------------------------------
            % Calculate proximity to the cap based on the maximum current correction
            current_max_corr = max(abs(sgd_correction));
            cap_proximity = min(1, current_max_corr / sgd_cap);
            
            % Apply Power-of-2 braking curve
            governor_factor = max(0.01, 1 - cap_proximity^2); 
            
            % Update the GLOBAL gain so it appears in history and plots
